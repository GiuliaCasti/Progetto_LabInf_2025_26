...
int main(int argc, char* argv[]) {
    ...
    double kappa = 0.01;                         // Coefficiente di diffusione
    
    std::string ordering = argv[2];             // true -> ordering.txt | false -> ordinamento naturale
    bool usaOrderingFile = (ordering == "true" || ordering == "1");

    // Definizione delle funzioni f(x, y) e g(x, y) come specificato nella traccia
    auto f = [](double x, double y) { return std::exp(-10 * (x * x + y * y)); }; // Termine noto f(x, y) = exp(-10(x^2+y^2))
    auto g = [](double x, double y) { return 0.0; }; // Condizione di Dirichlet sul bordo g(x, y) = 0
